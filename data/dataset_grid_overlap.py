...
def is_valid_directory(fn):
    try:
        return len(os.listdir(fn)) > 60
    except FileNotFoundError:
        print(f"Path not found: {fn}")
        return False
...
class GridSeq2Seq(Dataset):
    def __init__(self, opt, phase='train'):
        ...
        if phase == 'train':
            self.cur_data = self.read_paths_from_file('data/grid_split/overlap_train.txt')
        elif phase == 'val':
            self.cur_data = self.read_paths_from_file('data/grid_split/overlap_val.txt')
            np.random.seed(123)
            np.random.shuffle(self.cur_data)
            self.cur_data = self.cur_data[:opt.val_batch * opt.batch_size]
        else:
            self.cur_data = self.read_paths_from_file('data/grid_split/overlap_val.txt')
        self.cur_data = list(filter(is_valid_directory, self.cur_data))
    ...
    def __getitem__(self, index):
        item = self.cur_data[index]  # E:\dataset\GRID\lips\s31\swir4n
    ...
    def read_paths_from_file(self, file_path):
